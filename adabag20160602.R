...
iris.bagging.predmargins <- margins(iris.predbagging, iris[-train, ])
iris.bagging.margins
iris.bagging.predmargins
...
legend("topleft", c("test","train"), col = c("green", "blue"), lty = 1,
       lwd = 2)
...
legend("topleft", c("test", "train"), col = c("red", "blue"), lty = 1:2,
       lwd = 2)
...
data.prune <- predict.boosting(data.boosting, newdata = data[-train, ],
                               newmfinal = 253)
...
1 - sum(Vehicle.bagging$class == Vehicle$Class[sub]) /
  length(Vehicle$Class[sub])
Vehicle.bagging.pred <- predict.bagging(Vehicle.bagging,                 #논문에는 Vehicle.predbagging 이지만 Vehicle.bagging.pred 로 통일
                                          newdata = Vehicle[-sub, ])
